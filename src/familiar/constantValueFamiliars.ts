...
import { averageValue, value } from "../value";
...
const standardFamiliars: ConstantValueFamiliar[] = [
  {
    ...
    value: () => 0.02 * value($item`time's arrow`),
  },
  {
    familiar: $familiar`Stocking Mimic`,
    value: () =>
      averageValue(...$items`Polka Pop, BitterSweetTarts, Piddles`) / 6 +
  ...
  },
  {
    ...
    value: () =>
      value($item`elemental sugarcube`) / 5 +
  ...
  },
  {
    ...
    value: () => Math.min(value($item`twitching space egg`) * 0.0002, 690),
  },
  {
  ...
  },
  {
    ...
    value: ({ location }) =>
      location === $location`Crimbo Train (Caboose)` ? value($item`industrial lubricant`) / 11 : 0,
  },
  {
    familiar: $familiar`Cookbookbat`,
    value: () =>
      (3 *
        averageValue(...$items`Vegetable of Jarlsberg, Yeast of Boris, St. Sneaky Pete's Whey`)) /
      11,
  },
];
